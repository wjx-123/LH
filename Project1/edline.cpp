...
std::vector<cv::Rect> EDLines::getROIs(cv::Mat& image, int x, int y)
{
    std::vector<cv::Rect> rois;

    int width = image.cols / (x == 1 ? 1 : x++);
    int height = image.rows / (y == 1 ? 1 : y++);
...
Mat EDLines::drawOnImage(std::vector<LS>& lines)
{
...
cv::Mat EDLines::drawOwn(std::vector<LS>& lines)
{
    Mat colorImage = Mat(height, width, CV_8UC1, srcImg);
    cvtColor(colorImage, colorImage, COLOR_GRAY2BGR);
    for (int i = 0; i < lines.size(); i++) {
        line(colorImage, lines[i].start, lines[i].end, Scalar(0, 255, 0), 1, LINE_AA, 0); // draw lines as green on image
    }
    return colorImage;
}
