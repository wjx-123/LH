...
//
//	std::string classesFile = "class2.names";
...
//	ort_session = new Ort::Session(env, widestr.c_str(), sessionOptions);//加载模型
...
//
//	if (endsWith(config.modelpath, "best_smd.onnx"))
//	{
...
//}
//
//std::vector<std::pair<cv::Rect2f, cv::Rect2f>> YOLO::detect(cv::Mat& frame, cv::dnn::Net uNetModel, bool useUnet)
//{
//
...
//	}
//	//std::vector<std::vector<cv::Rect2f>> smt_frame = changeBoxInfoToRect(generate_boxes);//这行是把大图分割的方法，已弃用
//
//	std::vector<std::pair<cv::Rect2f, cv::Rect2f>> smt_frame;
//	if (useUnet == false)//不用unet模型
//	{
//		smt_frame = getBlackPosition(generate_boxes, frame);
//	}
//	else 
//	{
//		smt_frame = getBlackPosition_Unet(generate_boxes, frame, uNetModel);
//	}
//	generate_boxes.clear();
//	return smt_frame;
//}
//
//std::vector<cv::Rect2f> YOLO::splitImage(cv::Size2f image, int width, int height, int overlap)
//{
//	//std::vector<cv::Rect2f> rects;//imageSize.width就是cols imageSize.height就是rows
//	std::vector<cv::Rect2f> rects;
//
//	// 计算切割的行数和列数
//	int rows = (image.height - overlap) / (height - overlap);
//	int cols = (image.width - overlap) / (width - overlap);
//
//	// 处理最后一行和最后一列可能小于标准尺寸的情况
//	int lastRowHeight = image.height - rows * (height - overlap);
//	int lastColWidth = image.width - cols * (width - overlap);
//
//	for (int i = 0; i <= rows; ++i) {
//		for (int j = 0; j <= cols; ++j) {
//			int currentWidth = (j < cols || lastColWidth > width) ? width : lastColWidth;
//			int currentHeight = (i < rows || lastRowHeight > height) ? height : lastRowHeight;
//
//			float x = j * (width - overlap);
//			float y = i * (height - overlap);
//
//			cv::Rect2f rect(x, y, currentWidth, currentHeight);
...
//}
//
//std::vector<std::pair<cv::Rect2f, cv::Rect2f>> YOLO::getBlackPosition(std::vector<BoxInfo> generate_boxes, cv::Mat& frame)
//{
//	std::vector<std::pair<cv::Rect2f, cv::Rect2f>> result;
//	for (int i = 0; i < generate_boxes.size(); i++) 
//	{
//		if (generate_boxes[i].label != 0)//不是r
//		{
//			continue;
//		}
//		if (generate_boxes[i].x1 < 0 || generate_boxes[i].x2 < 0 || generate_boxes[i].y1 < 0 || generate_boxes[i].y2 < 0 )
...
//			continue;
//		}
//		cv::Rect2f smtRect = cv::Rect2f(std::min(generate_boxes[i].x1,generate_boxes[i].x2), 
//										std::min(generate_boxes[i].y1,generate_boxes[i].y2), 
//										std::abs(generate_boxes[i].x2 - generate_boxes[i].x1), 
//										std::abs(generate_boxes[i].y2 - generate_boxes[i].y1));
//	    //从大图中把这个小图扣出来
//		if (smtRect.x <= 0 || smtRect.y <= 0 || smtRect.width <= 0 || smtRect.height <= 0) 
//		{
//			continue;
//		}
//		cv::Mat smallSmtImg = frame(smtRect);
//		cv::Rect2f blackSmt = pathDectection.getBlackBlock(smallSmtImg);
//		if (!blackSmt.empty()) 
//		{
//			//if (std::max(blackSmt.width, blackSmt.height) < std::max(smtRect.width, smtRect.height) * 0.6)//黑块的最长边小于外框的最长边一半的时候，保留
//			//{
//				cv::Rect2f temp = {blackSmt.x + smtRect.x, blackSmt.y + smtRect.y, blackSmt.width, blackSmt.height};
//				result.push_back({ smtRect, temp });
//			//}
//		}
//	}
//	return result;
//}
//
//std::vector<std::pair<cv::Rect2f, cv::Rect2f>> YOLO::getBlackPosition_Unet(std::vector<BoxInfo> generate_boxes, cv::Mat& frame, cv::dnn::Net uNetModel)
//{
//	std::vector<std::pair<cv::Rect2f, cv::Rect2f>> result;
//	for (int i = 0; i < generate_boxes.size(); i++)
//	{
...
//		}
//		cv::Mat smallSmtImg = frame(smtRect);
//		std::vector<cv::Rect> getRectsOfSmall = uNetOnnxTemp.getDnnClassesRect(smallSmtImg, uNetModel);//得到子框的坐标
//		if (getRectsOfSmall.size() < 2)
//		{
//			continue;
//		}
//		cv::Rect2f temp1 = cv::Rect2f( getRectsOfSmall[0].x + smtRect.x, getRectsOfSmall[0].y + smtRect.y, getRectsOfSmall[0].width, getRectsOfSmall[0].height );
//		cv::Rect2f temp2 = cv::Rect2f(getRectsOfSmall[1].x + smtRect.x, getRectsOfSmall[1].y + smtRect.y, getRectsOfSmall[1].width, getRectsOfSmall[1].height);
